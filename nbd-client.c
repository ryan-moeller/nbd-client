...
static int
nbd_client_init(struct nbd_client *client, const char *host,
    struct addrinfo *ai)
{
	const int on = 1;
	int sock;
...
nbd_client_connect(struct nbd_client *client, cap_channel_t *capnet,
    const char *host, struct addrinfo *first_ai)
{
	struct addrinfo *ai;
	int sock;
...
nbd_client_dump(struct nbd_client *client)
{
	...
	syslog(LOG_DEBUG, "\tflags: %#010x [%s]", client->flags, flag_string);
...
nbd_newstyle_negotiation_dump(struct nbd_newstyle_negotiation *handshake)
{
	...
	syslog(LOG_DEBUG, "\thandshake_flags: %#06x [%s%s%s]%s", flags,
	    ...
	    ((flags & VALID_NEWSTYLE_FLAGS) == VALID_NEWSTYLE_FLAGS) ? "|" : "",
