...
usage()
{
	fprintf(stderr, "usage: %s "
		...
		"[[-A cacert] -C cert -K key] [-S sectorsize] "
		"host [port]\n",
		getprogname());
}
...
restart:
	while (connection_receiving(conn)) {
		...
		if (result == NBD_ESHUTDOWN) {
			result = connection_ggio_done(conn, ggio, ESHUTDOWN);
			if (result != SUCCESS)
				break;
			connection_degrade(conn, SOFT_DISCONNECTING);
			continue;
		...
		if (ggio->io.gctl_cmd == BIO_READ) {
			result =
			    nbd_client_recv_reply_data(conn->nbd,
						       ggio->io.gctl_length,
						       ggio->io.gctl_length,
						       ggio->io.gctl_data);
			if (result != SUCCESS) {
...
main(int argc, char *argv[])
{
	...
	cap_channel_t *capnet;
	struct addrinfo hints, *ai, *ai1;
	unsigned long maxphys;
	size_t page_size;
	uint64_t size;
	uint32_t minbs, prefbs;
	long sector_size;
	bool daemonize, list;
