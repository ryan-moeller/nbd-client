...
usage(void)
{
	fprintf(stderr, "usage: %s "
	    "[-fl] [-n export] [[-A cacert] -C cert -K key] host [port]\n",
	    getprogname());
}
...
loop_init(struct loop_context *ctx, ggate_context_t ggate, nbd_client_t nbd,
...
{
	if (ggioctl(ctx, G_GATE_CMD_START) == FAILURE)
		return (FAIL);
...
nbdcmd(struct loop_context *ctx)
{
	...
	case BIO_DELETE:
		return (nbd_client_send_trim(ctx->nbd,
		    ...
		    ctx->ggio.gctl_length));
	case BIO_FLUSH:
		return (nbd_client_send_flush(ctx->nbd,
		    ctx->ggio.gctl_seq));
	default:
...
loop_recv_data(struct loop_context *ctx)
{
	...
	result = nbd_client_recv_reply_data(ctx->nbd,
	    ctx->ggio.gctl_length, ctx->buflen, ctx->buf);
	if (result == FAILURE) {
		if (disconnect)
			return (FINISHED);
...
{
	if (ggioctl(ctx, G_GATE_CMD_DONE) == FAILURE) {
...
casper_enter_net(const char *host, const char *port)
{
...
main(int argc, char *argv[])
{
	...
	if (ggate_context_create_device(ggate, ident, size,
	    DEFAULT_SECTOR_SIZE, DEFAULT_GGATE_FLAGS) == FAILURE) {
